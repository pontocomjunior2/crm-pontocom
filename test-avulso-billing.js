...
const PackageService = require('./backend/services/packageService');
...
async function testAvulsoBilling() {
    ...
    await prisma.order.createMany({
        data: [
            { clientId: client.id, packageId: pkg.id, title: 'Audio 1', status: 'VENDA', serviceType: 'PACOTE DE AUDIOS', creditsConsumed: 1, vendaValor: 0, date: startDate },
            { clientId: client.id, packageId: pkg.id, title: 'Audio 2', status: 'VENDA', serviceType: 'PACOTE DE AUDIOS', creditsConsumed: 2, vendaValor: 0, date: startDate }, // 2 créditos
        ]
    });
    ...
    // Limpeza (opcional, vamos deixar para inspeção se falhar, ou deletar agora)
