...
import { recurringServiceAPI, orderAPI } from '../services/api';
...
const RecurringServiceList = () => {
    ...
    // New Tab State
    const [activeTab, setActiveTab] = useState('management'); // 'management' | 'history'
    const [historyOrders, setHistoryOrders] = useState([]);
    const [loadingHistory, setLoadingHistory] = useState(false);
    const [historyPagination, setHistoryPagination] = useState({ page: 1, limit: 50, total: 0, totalPages: 0 });
        ...
        <div className="flex flex-col h-full animate-in fade-in duration-500">
            ...
            <div className="flex bg-input-background rounded-xl p-1 border border-border mb-6 w-fit">
                <button
                    ...
                    className={`px-6 py-2.5 rounded-lg text-sm font-bold transition-all flex items-center gap-2 ${activeTab === 'management'
                ...
                <button
                    ...
                    className={`px-6 py-2.5 rounded-lg text-sm font-bold transition-all flex items-center gap-2 ${activeTab === 'history'
                        ? 'bg-primary text-primary-foreground shadow-lg shadow-primary/20'
                        : 'text-muted-foreground hover:text-foreground hover:bg-muted'
                        }`}
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <div className="relative md:col-span-2">
                    ...
                    <input
                        type="text"
                        placeholder={activeTab === 'management' ? "Buscar por nome ou cliente..." : "Buscar lançamento por título ou cliente..."}
                        value={searchTerm}
                        onChange={(e) => setSearchTerm(e.target.value)}
                        onKeyDown={(e) => {
                            if (e.key === 'Enter' && activeTab === 'history') {
                ...
                >
                    <RefreshCcw size={18} className={(loading || loadingHistory) ? 'animate-spin text-primary' : ''} />
                    Sincronizar
                </button>
            </div>
            ...
            {/* Content: History Tab */}
            {activeTab === 'history' && (
                <div className="flex-1 overflow-hidden flex flex-col min-h-0 bg-card rounded-2xl border border-border">
                                    ...
                                    {historyOrders.map((order) => (
                                        <tr key={order.id} className="hover:bg-white/5 transition-colors">
                                            ...
                                            <td className="px-6 py-4 text-center">
                                                {order.faturado ? (
                                                    <span className="inline-flex items-center gap-1.5 px-2 py-1 rounded bg-emerald-500/10 text-emerald-500 text-[10px] font-bold uppercase border border-emerald-500/20">
                                                        <CheckCircle2 size={12} /> Faturado
                                                    </span>
                                                ) : (
                                                    <span className="inline-flex items-center gap-1.5 px-2 py-1 rounded bg-amber-500/10 text-amber-500 text-[10px] font-bold uppercase border border-amber-500/20">
                    ...
                    <div className="p-4 border-t border-border flex items-center justify-between bg-card">
                        <span className="text-xs text-muted-foreground">
                            Página {historyPagination.page} de {historyPagination.totalPages || 1}
                        </span>
                        <div className="flex gap-2">
                            <button
                                disabled={historyPagination.page === 1}
                                onClick={() => setHistoryPagination(prev => ({ ...prev, page: prev.page - 1 }))}
                                className="px-3 py-1.5 rounded-lg border border-border text-xs font-bold disabled:opacity-50 hover:bg-white/5"
                            >
                                Anterior
                            </button>
                            <button
